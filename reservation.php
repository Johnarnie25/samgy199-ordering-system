...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(isset($_POST['submit'])) {
        ...
        if($guest != "" && $email && $phone != "" && $date_res != "" && $time != "" && $suggestions != "") {
            ...
            // Check for duplicate reservations with the same data
            $check = $db->query("SELECT * FROM reservation WHERE no_of_guest='$guest' AND email='$email' AND phone='$phone' AND date_res='$date_res' AND time='$time' AND user_id='$user_id' LIMIT 1");
            ...
            if($check->num_rows) {
                ...
                // Insert reservation into the database with user_id
                $insert = $db->query("INSERT INTO reservation(user_id, no_of_guest, email, phone, date_res, time, suggestions) VALUES('$user_id', '$guest', '$email', '$phone', '$date_res', '$time', '$suggestions')");
...
<title>SAMGYHAN 199 - RESERVATION</title>
	...
	<div class="inner_content">
		...
		<div class="contact">
			...
			<div class="icon_holder">
				...
				<a href="https://www.facebook.com/profile.php?id=61553215352222"><img src="image/icons/Facebook.png" alt="image/icons/Facebook.png" /></a>
